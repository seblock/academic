...
// ==============================================
// Functions slightly changed to apply to zoo. 

let buildSign = (type, name) => { return `Meet ${type} the ${name}!`; };
...
function PopulateSigns() {
  ...
  animals.forEach(animal => {
    let signResult = buildSign(animal.name,animal.type.name);
    let signCost = costOf(signResult);
    total += signCost;
    ...
    const list = document.getElementById('signDiv');
    if (list != null) {
      list.innerHTML += content;
    }
  ...
  const totalCost = document.getElementById('signCost');
