describe("Gigasecond", () => {
  ...
  // second it for date only specification of time
  it("tells another gigasecond anniversary since midnight", () => {
    const gs = gigasecond(new Date(Date.UTC(1977, 5, 13)));
    const expectedDate = new Date(Date.parse("2009-02-19T01:46:40Z"));
    expect(gs).toEqual(expectedDate);
  });
  // third it for date only specification of time
  it("tells gigasecond anniversary since midnight, from before UNIX epoch", () => {
    const gs = gigasecond(new Date(Date.UTC(1959, 6, 19)));
    const expectedDate = new Date(Date.parse("1991-03-27T01:46:40Z"));
    expect(gs).toEqual(expectedDate);
  });
  // full time specified
  it("tells the anniversary, including a time", () => {
